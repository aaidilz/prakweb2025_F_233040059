    ...
    <div class="space-y-6">
        ...
        <form action="{{ route('dashboard.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="col-span-2">
                ...
                <textarea name="body" id="body" rows="8" class="block w-full p-3 py-2.5 shadow-xs bg-neutral-secondary-medium border border-default focus:ring-brand focus:border-brand rounded-base focus:ring-brand focus:border-brand block w-full p-3 py-2.5 shadow-xs placeholder:text-body" placeholder="Write your post content here!">{{ old('body') }}</textarea>
                @error('body')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
